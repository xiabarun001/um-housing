...
固定信息永远不自动发布。实时信息自动发布：抓不到的小区沿用上一次的数和日期（页面上该小区的标记会写「沿用 X 抓到的数」），失败超过 5 个小区才整体作废、不改时间戳、不提交。
...
npm run status                  # 数据健康汇总，写 data/status.json
...
1. **不发布坏数据**：refresh.mjs 对 403 最多重试 5 次（10/20/30/40 秒递增），第一轮没抓到的小区歇 60 秒再补一轮；仍失败的沿用上一次的数和日期，失败超过 5 个小区才整体作废、不改时间戳、不提交；提交前再校验一次 prices.json（至少 20 个小区有价格、有更新时间）。固定信息永远只进 staging，人审后才发布。
