...
| 管理员 | Sasha 和 1–2 位同学 | 复核档案、审核采集结果、处理纠错、出图、管意向表 | 电脑 |
...
| US-6 | 作为管理员，我登录后台，在一个地方看到新鲜度、待审核、纠错、意向表和出图。 | P1 |
| US-7 | 作为管理员，新增一个小区只需要给一个 iProperty 项目链接。 | P1 |

## 4. 范围

**本期做**：三层模型落地（数据字段 + 页面标记 + 说明重写）；采集与审核流水线 v1；区域 3 六个小区；读者纠错入口；管理后台 v1（登录、总览、审核发布、纠错、意向表、出图）。
