...
4. **管理系统**：只登录、不注册，把采集审核和发布收进一个工作台（[ADR-003](ADR-003-管理后台.md)，放在最后做）。
...
| 管理员 | Sasha 和 1–2 位同学 | 复核档案、审核采集结果、发布改动 | 电脑 |
...
**本期做**：三层模型落地（数据字段 + 页面标记 + 说明重写）；采集与审核流水线 v1；区域 3 六个小区；管理系统 v1（登录、总览、审核发布、日志）。
...
```
/                指南（读者）
  我现在想要的房子 → 熟悉校园 → 熟悉三大区域 → 熟悉小区详情 → 计算租房费用 → 沟通中介信息 → 我最终想要的房子
/console/        UM 租房指南管理系统（管理员，登录后可进）
  总览 · 采集与审核 · 小区档案 · 日志
```
...
| P3 | 读者纠错 | 半天 |
| P4 | 管理系统 v1 | 3–5 天，需 Sasha 配 ADMIN_EMAILS 和 GitHub token |
...
- 不可触碰：`prices.json` 不手改；`condos.json` 里的指纹不改。
