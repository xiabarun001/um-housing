...
- [x] 每张卡片、详情弹窗、三种信息标记的气泡里都有「反馈」入口，点开是一个小表单：哪一类信息、具体哪一项、哪里不对、可选联系方式。
- [x] 提交写入 Supabase `reports` 表；匿名角色只能插入，不能读取（别人的联系方式不外泄）；数据库层校验长度和枚举；10 分钟内超过 30 条拒绝；页面端 30 秒内不能重复提交，带隐藏蜜罐字段。
- [x] 提交成功后告诉读者"7 天内处理，结果写进更新记录"；失败时提示稍后再试，不丢表单内容。
- [x] 管理端：`node scripts/reports.mjs list | done | reject | delete`（需要环境变量 `SUPABASE_SERVICE_KEY`，不进仓库）；改数据仍走 `publish.mjs` 留痕。
- [x] `status.mjs` 通过只暴露计数的 `report_counts()` 读取待处理数，写进 `data/status.json` 并告警。
- [ ] 待验收：7 天内处理率 ≥ 90%（上线后按月看）。
